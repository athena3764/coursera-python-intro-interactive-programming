...
class Sprite:
    ...
    def update(self):
        global state, a_rock,vel, angle_vel, angle
        self.angle += self.angle_vel
        self.pos[0] = (self.pos[0] + self.vel[0])%WIDTH
        self.pos[1] = (self.pos[1] + self.vel[1])%HEIGHT
...
def rock_spawner():
    global state, ang_vel,vel,pos, a_rock
    a_rock.pos = [random.randrange(WIDTH), random.randrange(HEIGHT)] 
    angle_lower = -0.05
    angle_upper = 0.06
    range_width = angle_upper -  angle_lower  
    lower = -0.3
    upper = 0.4
    ...
    if state == 0:
        state = 1
        a_rock.angle_vel = random.random() * range_width + angle_lower
        a_rock.vel = [random.random()*range_vel + lower,random.random()*range_vel + lower]
    elif state == 1:
        state = 0
      
def keydown(key):
    global thruster, angle_vel,forward, missile_group
