...
def new_game():
    global state, card1, card2, card, card_flipped, count
    card = list(range(1,9) + range(1,9))
    card_flipped = list(range(1,9) + range(1,9))
    state = 0
    card1 = 0
    card2 = 0
    count = 0
...
def mouseclick(pos):
    ...
    #check if the first and second cards match, and if not, unflip 
    #them. Regardless of what happened in that check, flip current 
    #card, save it as the first card and next state is 1
    else:
        state = 1
...
def draw(canvas):
    global card, card_flipped
    for card_index in range(len(card)):
        card_pos = 50*card_index
        
        #draw exposed cards
        if card_flipped[card_index] == False:
